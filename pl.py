...
cl = LINETCR.LINE()
#cl.login(qr=True)
cl.login(token='Ep3on0yUn7b63PVaFRXb.ue3KGPZ+Mj3Xuj/0H0vu/W.laqeBEexJUNc+1WyQdShE+P4c1tZK1f9jwIgv0GiYIc=')
cl.loginResult()
print "Cl-Login Success\n"

ki = LINETCR.LINE()
#ki.login(qr=True)
ki.login(token='Ep7bkiGsF50LGHDxYsib.JdoCWvVgaXxi03Z3ppd3IW.i+KBfALKJUP5vCIhLtV4gUzjzFFm3VB28MNzFzZm5A8=')
ki.loginResult()
print "Ki-Login Success\n"

kk = LINETCR.LINE()
#kk.login(qr=True)
kk.login(token='EptySF84LYFBUGGjlwz7.kRhJLdrjqx7rEiydurJFzW.nK1+pRd4PWTUM6QuJBIL3am8/rsXqhVYi1zCVX2NksE=')
kk.loginResult()
print "Kk-Login Success\n"

kc = LINETCR.LINE()
#kc.login(qr=True)
kc.login(token='EpC0joNjTacZl5WCxN51.tw8ZOFCFrGCQo+5vI/Dl0q.XOhF2owBqbjx/i45XHAch2O+OLQE+srzc+itqwqomhw=')
kc.loginResult()
print "Kc-Login Success\n"

kr = LINETCR.LINE()
#kr.login(qr=True)
kr.login(token='EpaciHnh4S02QJVV3be8.Y0PUG3bMKzf25rjlCYfyIa.B6jJeN6+WwujOMbiCCkXDoz0m7PHlyXbkprdWN1NR8g=')
kr.loginResult()
print "Kr-Login Success\n"

km = LINETCR.LINE()
#km.login(qr=True)
km.login(token='Ep8HDECxCbcViMVfVsXe.ds6pSK3N+KxkU1bgTYwWJG.kQg+LpJnNWh9dlBhN6qckovL4fo4a23V1QLjyMLssW4=')
km.loginResult()
...
╚═════════════════════════
"""
tjia="ue8dfb935c47521f44fbe6f8a1086b40a"
#----------------------------------------------------------
setMessage ="""
╔═════════════════════════
║              『┅༅💎 ֆ ɛ ȶ 💎༅┅』
...
Creator=["ue8dfb935c47521f44fbe6f8a1086b40a"]
admin=["ue8dfb935c47521f44fbe6f8a1086b40a"]
...
wait = {
    ...
    "comment4":"Nice Kak (^_^)",
    "comment5":"Bot Auto Like ©By : Plucyvrz",    
    "commentOn":True,
    "commentBlack":{},
    "likeOn":False,
    "blacklist":{},
                        ...
                        if op.param1 in cctv['point']:
                            ...
                            if Name in cctv['sidermem'][op.param1]:
                                ...
                                if " " in Name:
                                    nick = Name.split(' ')
                                    if len(nick) == 2:
                                        kc.sendText(op.param1, "Jangan Jadi sider lah " +nick[0])
                                        #random.choice(KAC).kickoutFromGroup(op.param1, [op.param2])
                                    else:
                                        kc.sendText(op.param1, "Keciduk kamu " +nick[1])
                                        #random.choice(KAC).kickoutFromGroup(op.param1, [op.param2])
                                else:
                                    kc.sendText(op.param1, "Cie ngintip " +Name)
                                    #random.choice(KAC).kickoutFromGroup(op.param1, [op.param2])
                        else:
                            pass
                    else:
...
#--------------------------------------------------------
              ...
              if msg.from_ in admin:
            	...
            	cl.sendText(msg.to, "Hayo mau ngapain. .  ")
...
#--------------------------------------------------------
		...
		if msg.from_ in Creator:
		    ...
		    cl.sendText(msg.to, "Khusus Plucy")
#--------------------------------------------------------
...
#--------------------------------------------------------
                ...
                midd = msg.text.replace("Invite creator ","ue8dfb935c47521f44fbe6f8a1086b40a")
                cl.findAndAddContactsByMid(midd)
                cl.inviteIntoGroup(msg.to,[midd])
#--------------------------------------------------------
...
#--------------------------------------------------------
		...
		if msg.from_ in admin:
		    for i in gid:
			cl.sendText(i,"=======[BROADCAST]=======\n\n"+bc+"\n\nContact Me : line.me/ti/p/~plucyvrz_.")
...
#--------------------------[Youtube]---------------------
            elif 'Ytvideo ' in msg.text:
                try:
                    textToSearch = (msg.text).replace('Ytvideo ', "").strip()
